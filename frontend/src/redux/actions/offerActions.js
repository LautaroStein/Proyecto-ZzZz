    ...
    getOffers: () => {
        return async (dispatch, getState) => {
            try {
                ...
                const filteredOffers = offers.data.response.filter(offer => offer.valid)
                dispatch({ type: 'GET_OFFERS', payload: filteredOffers })
            } catch (error) {
                console.log(error);
            }
        }
    ...
    updateOffer: (offerId, paramOffer) => {
        return async (dispatch, getState) => {
            try {
                ...
                dispatch({ type: 'UPDATE_OFFER', payload: { offerId: offer.data.response, body: paramOffer } })
            } catch (error) {
                console.log(error);
            }
        }
